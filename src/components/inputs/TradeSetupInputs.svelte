<script lang="ts">
    ...
    export let entryPrice: string | null;
    export let useAtrSl: boolean;
    export let atrValue: string | null;
    export let atrMultiplier: string | null;
    export let stopLossPrice: string | null;
    ...
    function handleEntryPriceInput(e: Event) {
        ...
        updateTradeStore(s => ({ ...s, entryPrice: value === '' ? null : value }));
    }

    function handleAtrValueInput(e: Event) {
        const target = e.target as HTMLInputElement;
        const value = target.value;
        updateTradeStore(s => ({ ...s, atrValue: value === '' ? null : value }));
    }

    function handleAtrMultiplierInput(e: Event) {
        const target = e.target as HTMLInputElement;
        const value = target.value;
        updateTradeStore(s => ({ ...s, atrMultiplier: value === '' ? null : value }));
    }

    function handleStopLossPriceInput(e: Event) {
        const target = e.target as HTMLInputElement;
        const value = target.value;
        updateTradeStore(s => ({ ...s, stopLossPrice: value === '' ? null : value }));
    }
</script>
...
<div>
    ...
    <input id="entry-price-input" type="text" use:numberInput={{ maxDecimalPlaces: 4, minValue: 0 }} value={entryPrice ?? ''} on:input={handleEntryPriceInput} class="input-field w-full px-4 py-2 rounded-md mb-4" placeholder="{$_('dashboard.tradeSetupInputs.entryPricePlaceholder')}" on:input={onboardingService.trackFirstInput}>
    ...
    <div class="p-2 rounded-lg mb-4" style="background-color: var(--bg-tertiary);">
        ...
        {#if !useAtrSl}
            <div>
                <input id="stop-loss-price-input" type="text" use:numberInput={{ maxDecimalPlaces: 4, minValue: 0 }} value={stopLossPrice ?? ''} on:input={handleStopLossPriceInput} class="input-field w-full px-4 py-2 rounded-md" placeholder="{$_('dashboard.tradeSetupInputs.manualStopLossPlaceholder')}">
            </div>
        {:else}
            {#if atrMode === 'manual'}
                <div class="grid grid-cols-2 gap-2 mt-2">
                    <input id="atr-value-input" type="text" use:numberInput={{ maxDecimalPlaces: 4, minValue: 0 }} value={atrValue ?? ''} on:input={handleAtrValueInput} class="input-field w-full px-4 py-2 rounded-md" placeholder="{$_('dashboard.tradeSetupInputs.atrValuePlaceholder')}">
                    <input id="atr-multiplier-input" type="text" use:numberInput={{ maxDecimalPlaces: 4, minValue: 0 }} value={atrMultiplier ?? ''} on:input={handleAtrMultiplierInput} class="input-field w-full px-4 py-2 rounded-md" placeholder="{$_('dashboard.tradeSetupInputs.multiplierPlaceholder')}">
                </div>
            {:else}
                <div class="grid grid-cols-3 gap-2 mt-2 items-end">
                    ...
                    <div>
                        ...
                        <input id="atr-value-input-auto" type="text" use:numberInput={{ maxDecimalPlaces: 4, minValue: 0 }} value={atrValue ?? ''} on:input={handleAtrValueInput} class="input-field w-full px-4 py-2 rounded-md" placeholder="ATR">
                    </div>
                    <div>
                        <label for="atr-multiplier-input-auto" class="input-label !mb-1 text-xs">{$_('dashboard.tradeSetupInputs.atrMultiplierLabel')}</label>
                        <input id="atr-multiplier-input-auto" type="text" use:numberInput={{ maxDecimalPlaces: 4, minValue: 0 }} value={atrMultiplier ?? ''} on:input={handleAtrMultiplierInput} class="input-field w-full px-4 py-2 rounded-md" placeholder="1.5">
                    </div>
                </div>
            {/if}
