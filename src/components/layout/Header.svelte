<script lang="ts">
    ...
    function handlePresetLoad(event: Event) {
        ...
        dispatch('presetLoad', selectedPreset);
    }

    // This reactive variable formats the theme name for the tooltip.
    // e.g., 'solarized-light' becomes 'Solarized Light'
...
<div class="flex flex-col md:flex-row md:justify-between md:items-center mb-6 gap-4">
    <div class="flex justify-between items-center w-full md:w-auto">
        ...
        <button
            ...
            on:click={() => dispatch('viewJournal')}
            use:trackClick={{ category: 'Navigation', action: 'Click', name: 'ViewJournalMobile' }}
        >
            {$_('app.journalButton')}
        </button>
    </div>
    <div class="flex items-center flex-wrap justify-end gap-2 w-full md:w-auto">
        <div class="flex items-center flex-wrap justify-end gap-2 md:order-1">
            ...
            <button
                ...
                on:click={() => dispatch('savePreset')}
                use:trackClick={{ category: 'Presets', action: 'Click', name: 'SavePreset' }}
            >
                {@html icons.save}
            </button>
            <button
                ...
                on:click={() => dispatch('deletePreset')}
                use:trackClick={{ category: 'Presets', action: 'Click', name: 'DeletePreset' }}
            >
                {@html icons.delete}
            </button>
            <button
                ...
                use:trackClick={{ category: 'Actions', action: 'Click', name: 'ResetAll' }}
            >
                {@html icons.broom}
            </button>
            <button
                ...
                on:click={() => dispatch('switchTheme')}
                title={themeTitle}
                use:trackClick={{ category: 'Settings', action: 'Click', name: 'SwitchTheme' }}
            >
                {@html themeIcons[$uiStore.currentTheme as keyof typeof themeIcons]}
            </button>
        </div>
        <button
            ...
            title="{$_('app.journalButtonTitle')}"
            on:click={() => dispatch('viewJournal')}
            use:trackClick={{ category: 'Navigation', action: 'Click', name: 'ViewJournalDesktop' }}
        >
            {$_('app.journalButton')}
        </button>
    </div>
