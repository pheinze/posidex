...
import { debounce, parseDecimal } from './utils';
import { CONSTANTS, themes } from './constants';
import { apiService } from './apiService';
import { modalManager } from './modalManager';
import { uiManager } from './uiManager';
import { calculator } from './calculator';
import { appStore, updateStore, clearResults, showError, hideError, showFeedback, setTheme, updateSymbolSuggestions, toggleAtrInputs, resetAllInputs, toggleJournalModal, journalStore, type JournalEntry } from './stores';
import type { TradeValues, IndividualTpResult } from './calculator';
...
export const app = {
    ...
    calculateAndDisplay: () => {
        ...
        const getAndValidateInputs = (): { status: string; message?: string; fields?: string[]; data?: TradeValues } => {
            ...
            if (values.useAtrSl) {
                if (values.entryPrice.gt(0) && values.atrValue.gt(0) && values.atrMultiplier.gt(0)) {
                    ...
                    updateStore(state => ({ ...state, showAtrFormulaDisplay: true, atrFormulaText: `SL = ${values.entryPrice.toFixed(4)} ${operator} (${values.atrValue} × ${values.atrMultiplier}) = ${values.stopLossPrice.toFixed(4)}` }));
        ...
        updateStore(state => ({ ...state, calculatedTpDetails: calculatedTpDetails }));
    ...
    getJournal: (): any[] => {
        ...
        try {
            ...
            showError("Journal konnte nicht geladen werden.");
            return [];
        }
    },
    saveJournal: (d: any[]) => {
        ...
        try {
            ...
            showError("Fehler beim Speichern des Journals. Der lokale Speicher ist möglicherweise voll oder blockiert.");
        }
    },
    addTrade: () => {
        const currentAppState = get(appStore);
        if (!currentAppState.currentTradeData.positionSize || currentAppState.currentTradeData.positionSize.lte(0)) { showError("Kann keinen ungültigen Trade speichern."); return; }
        const journalData = app.getJournal();
        journalData.push({ ...currentAppState.currentTradeData, notes: currentAppState.tradeNotes, id: Date.now(), date: new Date().toISOString() });
    ...
    async clearJournal() {
        ...
        if (await modalManager.show("Journal leeren", "Möchten Sie wirklich das gesamte Journal unwiderruflich löschen?", "confirm")) {
            app.saveJournal([]);
            journalStore.set([]); 
            showFeedback('save', 2000, "Journal geleert!");
        }
    },
    ...
    savePreset: async () => {
        ...
        try {
            ...
            showFeedback('save');
            app.populatePresetLoader();
            updateStore(state => ({ ...state, selectedPreset: presetName }));
        } catch (e) {
            console.error("Fehler beim Speichern des Presets:", e);
            showError("Preset konnte nicht gespeichert werden. Der lokale Speicher ist möglicherweise voll oder blockiert.");
        }
    },
    deletePreset: async () => {
        if (!browser) return;
        ...
        try {
            ...
            localStorage.setItem(CONSTANTS.LOCAL_STORAGE_PRESETS_KEY, JSON.stringify(presets));
            app.populatePresetLoader();
            updateStore(state => ({ ...state, selectedPreset: '' }));
        } catch (e) { showError("Preset konnte nicht gelöscht werden."); }
    },
    loadPreset: (presetName: string) => {
        ...
        try {
            ...
            showError("Preset konnte nicht geladen werden.");
        }
    },
    populatePresetLoader: () => {
        ...
        try {
            ...
            updateStore(state => ({ ...state, availablePresets: presetNames }));
        } catch (e) {
            console.warn("Could not populate presets from localStorage.", e);
            updateStore(state => ({ ...state, availablePresets: [] }));
        }
    },
    exportToCSV: () => {
        if (!browser) return;
        const journalData = get(journalStore);
        if (journalData.length === 0) { showError("Journal ist leer."); return; }
    ...
    importFromCSV: (file: File) => {
        ...
        reader.onload = async (e) => {
            ...
            if (lines.length < 2) {
                showError("CSV ist leer oder hat nur eine Kopfzeile.");
                return;
            }
            ...
            if (missingHeaders.length > 0) {
                showError(`CSV-Datei fehlen benötigte Spalten: ${missingHeaders.join(', ')}`);
                return;
            }
            ...
            if (entries.length > 0) {
                ...
                if (await modalManager.show("Import bestätigen", `Sie sind dabei, ${entries.length} Trades zu importieren. Bestehende Trades mit derselben ID werden überschrieben. Fortfahren?`, "confirm")) {
                    ...
                    showFeedback('save', 2000, "Import erfolgreich!");
                }
            } else {
                showError("Keine gültigen Einträge in der CSV-Datei gefunden.");
            }
        };
    ...
    handleFetchPrice: async () => {
        ...
        try {
            ...
            showFeedback('copy', 700, 'Preis geladen!'); // Reusing copy feedback for price loaded animation
            app.calculateAndDisplay();
        } catch (error: any) {
    ...
    adjustTpPercentages: (changedIndex: number | null) => {
        ...
        updateStore(state => ({ ...state, targets: targets }));
    },
};
